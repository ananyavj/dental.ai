...
export function DashboardPage() {
  ...
  const doctorName =
    profile?.full_name?.replace(/^Dr\.?\s+/i, "") || "your name";
          ...
          <CardContent className="space-y-4">
            ...
            {cases.map((item) => (
              <button
                ...
                className="flex w-full items-start gap-4 rounded-xl border border-border bg-muted/30 p-4 text-left"
              >
                <div
                  className={`mt-1 h-3 w-3 rounded-full ${item.severity === "EMERGENCY" ? "bg-danger" : item.severity === "URGENT" ? "bg-warning" : "bg-success"}`}
                />
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    {item.specialty} • {formatDateTime(item.last_activity_at)}
                  </p>
                </div>
              </button>
            ))}
              ...
              {appointments.map((item) => (
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-foreground">
                    ...
                    {item.clinic_location ? ` • ${item.clinic_location}` : ""}
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">
            ...
            <CardContent className="space-y-4">
              ...
              {activity.map((item) => (
                <div key={item.id} className="rounded-xl bg-muted/30 p-4">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {item.severity} • {formatDateTime(item.created_at)}
                  </p>
                </div>
              ))}
            </CardContent>
