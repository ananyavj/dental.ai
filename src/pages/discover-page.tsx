...
export default function DiscoverPage() {
  ...
  const researchCards = useMemo(() => papers.slice(0, 12), [papers])
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {caseStudies.map(study => (
            <button key={study.id} type="button" onClick={() => navigate(`/discover/case-studies/${study.id}`)} className="text-left">
              <Card className="h-full transition duration-150 hover:border-primary/40">
                <CardContent className="flex h-full flex-col">
                  <div className="flex items-center justify-between gap-3">
                    <Badge>{study.specialty}</Badge>
                    <p className="text-xs text-muted-foreground">{new Date(study.publishedAt).toLocaleDateString('en-IN')}</p>
                  </div>
