...
export async function getDashboardData(profile?: Profile | null): Promise<{
  ...
  const cases = await queryOrFallback(async () => {
  ...
  const appointments = await queryOrFallback(async () => {
    const { data } = await supabase!
      .from("appointments")
    ...
    return (
      ((data || []).map((item: Record<string, unknown>) => ({
        ...
        patient_name:
          ...
          "Patient",
        contact_number:
          (item.patients as { contact_number?: string } | null)
            ?.contact_number || null,
      })) as Appointment[]) || demoAppointments
    );
  }, demoAppointments);

  const activity = await queryOrFallback(async () => {
    const { data } = await supabase!
      .from("audit_events")
      .select("*")
    ...
    metrics: [
      {
        ...
        value: cases.length,
        helper: "From your clinic workspace",
      },
      {
        label: "Urgent cases",
        value: cases.filter((item) => item.severity === "URGENT").length,
        helper: "Fast same-day prioritization",
      },
      {
        label: "Saved drafts",
        value: activity.length,
        helper: "Referrals, treatment plans, and audit events",
      },
      {
        label: "Conversations",
        value: conversations.length,
        helper: "Practitioner, student, and patient modes",
      },
    ],
    cases,
    appointments,
    ...
    async () => {
      ...
      return rows.length
        ? [
            ...rows,
            ...localDrafts.filter(
              (local) => !rows.some((remote) => remote.id === local.id),
            ),
          ]
        : [...localDrafts, ...demoAppointments];
    },
    localDrafts.length
      ? [...localDrafts, ...demoAppointments]
      : demoAppointments,
...
export async function scheduleAppointmentFromTriage(
  profile: Profile,
  payload: {
    patientId: string;
    patientName: string;
  ...
  if (!supabase || !isSupabaseConfigured) {
    const drafts = readStorage<Appointment[]>(
      APPOINTMENT_DRAFTS,
      demoAppointments,
    );
...
export async function getChatConversations(profile: Profile) {
  ...
  const fallback = localDrafts.length
    ? localDrafts
...
export async function upsertConversation(
  ...
  if (!supabase || !isSupabaseConfigured) {
    const drafts = readStorage<Conversation[]>(
      CONVERSATION_DRAFTS,
      demoConversations,
    );
  ...
  return queryOrFallback(
    async () => {
      const { data } = await supabase!
        .from("knowledge_base")
        .select("*")
        .contains("metadata", { content_type: "case_study" });
      const studies = (data || []).map((item) => {
        ...
        return {
          ...
          authorRole: String(metadata.author_role || "doctor") as
            | "doctor"
            | "student"
            | "patient"
            | "admin"
          ...
          summary: String(
            metadata.summary || stripHtml(item.content).slice(0, 180),
          ),
          ...
          contentType: String(metadata.content_type || "case_study") as
            | "case_study"
            | "study_guide"
            | "workflow",
...
export async function publishCaseStudy(
  ...
  study: Omit<
    CaseStudy,
    "id" | "authorName" | "authorId" | "publishedAt" | "views" | "helpfulCount"
  >,
) {
  ...
  if (!supabase || !isSupabaseConfigured) {
    const current = readStorage<CaseStudy[]>(
      CASE_STUDY_DRAFTS,
      demoCaseStudies,
    );
    writeStorage(CASE_STUDY_DRAFTS, [
      draft,
...
export async function saveReferral(
  ...
  await appendAudit(profile, {
    event_type: "referral",
    event_title: `Saved referral for ${payload.patientName}`,
    severity: "ROUTINE",
  });
}
...
export async function saveTreatmentPlan(
  ...
  const draft: SavedTreatmentPlan = {
    id: crypto.randomUUID(),
    patient_name: payload.patientName,
    patient_gender: payload.patientGender || null,
    urgency: payload.urgency,
    created_at: now,
    plan_json: contextPlan,
  };

  if (!supabase || !isSupabaseConfigured) {
    const drafts = readStorage<SavedTreatmentPlan[]>(TREATMENT_PLAN_DRAFTS, []);
    writeStorage(TREATMENT_PLAN_DRAFTS, [draft, ...drafts]);
    return;
  }
  ...
  if (error) {
    ...
    if (retryError) {
      const drafts = readStorage<SavedTreatmentPlan[]>(
        TREATMENT_PLAN_DRAFTS,
        [],
      );
      writeStorage(TREATMENT_PLAN_DRAFTS, [draft, ...drafts]);
      return;
  ...
  await appendAudit(profile, {
    ...
    event_title: `Saved treatment plan for ${payload.patientName}`,
    severity: "ROUTINE",
  });
}

export async function getSavedTreatmentPlans(profile?: Profile | null) {
  const localDrafts = readStorage<SavedTreatmentPlan[]>(
    TREATMENT_PLAN_DRAFTS,
    [],
  );
  ...
  return queryOrFallback(async () => {
    ...
    const remote = ((data || []) as Record<string, unknown>[]).map((item) => {
      const plan = (item.plan_json || {}) as Record<string, unknown>;
      const patientContext = (plan.patient_context || {}) as Record<
        string,
        unknown
      >;
      return {
        id: String(item.id),
        patient_name: String(item.patient_name || "Patient"),
        patient_gender:
          String(item.patient_gender || patientContext.gender || "") || null,
        urgency: (item.urgency as string | null) || null,
        created_at: String(item.created_at || new Date().toISOString()),
        plan_json: item.plan_json,
      } satisfies SavedTreatmentPlan;
    });
    ...
    return remote.length
      ? [
          ...remote,
          ...localDrafts.filter(
            (local) => !remote.some((server) => server.id === local.id),
          ),
        ]
      : localDrafts;
  }, localDrafts);
...
export async function saveXrayReport(
  profile: Profile,
  payload: {
    caseId?: string;
    fileName: string;
  ...
  await appendAudit(profile, {
    event_type: "xray",
    event_title: `Saved imaging report: ${payload.fileName}`,
    severity: "URGENT",
  });
}
