...
export async function getDashboardData(profile?: Profile | null): Promise<{
  ...
  const appointments = await queryOrFallback(
    async () => {
      const { data } = await supabase!.from('appointments').select('*').order('appointment_date', { ascending: true }).limit(4)
      return (data as Appointment[]) || demoAppointments
    },
    demoAppointments
  )
...
export async function getCommunityCaseStudies() {
  return queryOrFallback(
    async () => {
      ...
      const merged = [...localStudies, ...studies]
